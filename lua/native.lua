...
local arch_aliases = {['x64'] = 'x86_64', ['arm64'] = 'aarch64'}
...
local dirname = string.sub(debug.getinfo(1).source, 2,
                           string.len('/native.lua') * -1)
local library_path = dirname .. '../static/libfzy-' .. os .. '-' .. arch ..
                         '.so'
...
local function positions_to_lua(positions, length)
    local result = {}
    for i = 0, length - 1, 1 do table.insert(result, positions[i] + 1) end
    return result
end
...
function fzy.score(needle, haystack)
    local is_case_sensitive = false
    local score = native.match_positions(needle, haystack, nil,
                                         is_case_sensitive)
    return score
end
...
function fzy.positions(needle, haystack)
    ...
    -- TODO: return score
    local score = native.match_positions(needle, haystack, positions,
                                         is_case_sensitive)
...
function fzy.positions_many(needle, haystacks)
    ...
    local haystacks_arg = ffi.new("const char*[" .. (length + 1) .. "]",
                                  haystacks)

    -- TODO: return score
    local score = native.match_positions_many(needle, haystacks_arg, length,
                                              scores, positions,
                                              is_case_sensitive)
...
function fzy.get_score_floor() return (MATCH_MAX_LENGTH + 1) * SCORE_GAP_INNER end
